...
if (!ARENA_ADDRESS)
    throw new Error("Missing ARENA_ADDRESS");
...
var ActionType;
(function (ActionType) {
    ActionType[ActionType["SET_NAME"] = 0] = "SET_NAME";
    ActionType[ActionType["JOIN"] = 1] = "JOIN";
    ActionType[ActionType["MOVE"] = 2] = "MOVE";
})(ActionType || (ActionType = {}));
function commonDomain(chainId, arenaAddress) {
    return ethers_1.ethers.keccak256(ethers_1.ethers.solidityPacked(["uint256", "address"], [chainId, arenaAddress]));
}
function digestJoin(params) {
    const domain = commonDomain(CHAIN_ID, ARENA_ADDRESS);
    return ethers_1.ethers.keccak256(ethers_1.ethers.solidityPacked(["bytes32", "uint8", "address", "uint256", "uint256"], [domain, ActionType.JOIN, params.player, params.nonce, params.deadline]));
}
function digestMove(params) {
    const domain = commonDomain(CHAIN_ID, ARENA_ADDRESS);
    return ethers_1.ethers.keccak256(ethers_1.ethers.solidityPacked(["bytes32", "uint8", "address", "uint8", "uint256", "uint256"], [domain, ActionType.MOVE, params.player, params.dir, params.nonce, params.deadline]));
}
function digestSetName(params) {
    const domain = commonDomain(CHAIN_ID, ARENA_ADDRESS);
    return ethers_1.ethers.keccak256(ethers_1.ethers.solidityPacked(["bytes32", "uint8", "address", "bytes12", "uint256", "uint256"], [
        domain,
        ActionType.SET_NAME,
        params.player,
        params.nameBytes12,
...
function verifyBurnerSignature(opts) {
    const recovered = ethers_1.ethers.verifyMessage(ethers_1.ethers.getBytes(opts.digest), opts.sig);
    return recovered.toLowerCase() === opts.player.toLowerCase();
}
function isAddr(x) {
    try {
        ethers_1.ethers.getAddress(x);
        return true;
    }
    catch {
        return false;
...
const wallet = new ethers_1.ethers.Wallet(RELAYER_PRIVATE_KEY, provider);
// NonceManager prevents nonce races inside this single process
const signer = new ethers_1.ethers.NonceManager(wallet);
const arena = new ethers_1.ethers.Contract(ARENA_ADDRESS, ARENA_ABI, signer);
...
async function enqueueMoveOnce(player, fn) {
    const key = player.toLowerCase();
    const existing = moveInFlight.get(key);
    if (existing)
        return { deduped: true, promise: existing };
...
app.use((0, cors_1.default)({
    origin: (origin, cb) => {
        ...
        if (ALLOWED_ORIGINS.length === 0)
            return cb(null, true); // allow all if not set
...
app.get("/health", async (_req, res) => {
    try {
        ...
        res.json({
            ...
            relayerBalance: ethers_1.ethers.formatEther(bal),
            arena: ARENA_ADDRESS,
        });
    }
    catch (e) {
...
app.post("/join", async (req, res) => {
    try {
        const { player, nonce, deadline, sig } = req.body;
        if (!isAddr(player))
            return res.status(400).json({ ok: false, error: "Bad player address" });
        if (typeof sig !== "string" || !sig.startsWith("0x"))
            return res.status(400).json({ ok: false, error: "Bad sig" });
...
app.post("/set-name", async (req, res) => {
    try {
        const { player, nameBytes12, nonce, deadline, sig } = req.body;
        if (!isAddr(player))
            return res.status(400).json({ ok: false, error: "Bad player address" });
...
app.post("/move", async (req, res) => {
    try {
        const { player, dir, nonce, deadline, sig } = req.body;
        if (!isAddr(player))
            return res.status(400).json({ ok: false, error: "Bad player address" });
        const d = Number(dir);
        ...
        const { deduped, promise } = await enqueueMoveOnce(player, () => arena.moveFor(player, d, n, dl, sig, { gasLimit: 300000n }));
